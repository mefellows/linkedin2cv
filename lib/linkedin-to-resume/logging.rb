  ...
  class << self
    ...
    def configure_logger_for(classname)
      ...
      if (ENV['LOG_OUTPUT_FILENAME'] && !ENV['LOG_OUTPUT_FILENAME'].empty?)
        ...
        logger.outputters << Log4r::FileOutputter.new('linkedin-to-resumelog', :filename => ENV['LOG_OUTPUT_FILENAME'])
      elsif
        logger.outputters << Log4r::StdoutOutputter.new('linkedin-to-resumelog')
      end
      logger
    end
  end
